...
function BoardCopy(props) {
  ...
  const showCategory = () => {
    setCategoryList(['전체'])
    axios.get('api/show/category/' + user.id).then(res => {
  ...
  const selectedCategoryHandle = data => {
    if (selectedCategory.includes(data)) {
      ...
      setSelectedCategory(
        selectedCategory.filter((value, index) => index != num)
      )
      console.log('삭제')
    } else {
    ...
    <div className="flex min-h-screen bg-byuncolor">
      ...
      <div className={'w-0'}>
        ...
        <div className="w-full flex fixed bg-white mt-16 py-2 z-20 ">
            ...
            {categoryList &&
              categoryList.map(data => {
                return (
                  <div>
                    {category === data ? (
                      <div className="bg-blue-500 text-white py-1 rounded-xl mx-2 px-3">
                        {data}
                      </div>
                    ) : (
                      <div
                        onClick={() => categoryHandle(data)}
                        className="bg-gray-200 py-1 rounded-xl mx-2 px-3 hover:bg-gray-300"
                      >
                        {data}
                      </div>
                    )}
          ...
          <button
            className="mr-10 bg-blue-500 text-white font-bold  px-2 rounded-xl hover:bg-indigo-500"
            onClick={ModalOpen}
          >
            {' '}
            카테고리 관리{' '}
      ...
      <div className="w-full flex mt-16 z-10">
        ...
        >
          <Box className="bg-white w-192 mx-auto mt-5 h-min rounded-xl p-3 relative">
            ...
            <div className="mt-5 bg-yellow-100 rounded-t-xl py-1 font-bold border-t border-l border-r px-4">
              엔터테이먼트,예술
            </div>
                ...
                return (
                  <div className="">
                    {selectedCategory.includes(data) ? (
                      <p
                        onClick={() => selectedCategoryHandle(data)}
                        className="rounded-2xl px-3 py-1 text-white bg-blue-500 my-1 mx-2"
                      >
                        {data}
                      </p>
                    ) : (
                      <p
                        onClick={() => selectedCategoryHandle(data)}
                        className="rounded-2xl px-3 my-1 py-1 bg-gray-200 mx-2 hover:bg-gray-300"
                      >
                        {data}
                      </p>
                    )}
            ...
            <div className="mt-10 bg-green-100 rounded-t-xl py-1 font-bold border-t border-l border-r px-4">
              생활,노하우,쇼핑
            </div>
            <div className="flex bg-gray-100 flex-wrap rounded-b-xl pt-4 pb-4 justify-center border">
              {life.map(data => {
            ...
            <div className="mt-10 bg-red-100 rounded-t-xl py-1 font-bold border-t border-l border-r px-4">
              취미,여가,여행
            </div>
            <div className="flex bg-gray-100 flex-wrap rounded-b-xl pt-4 pb-4 justify-center border">
              {hobby.map(data => {
            ...
            <div className="mt-10 bg-gray-300 rounded-t-xl py-1 font-bold border-t border-l border-r px-4">
              지식,동향
            </div>
            <div className="flex bg-gray-100 flex-wrap rounded-b-xl pt-4 pb-4 justify-center border">
              {know.map(data => {
            ...
            <button
              ...
              className="bg-green-200 p-3 rounded-xl hover:bg-green-400 font-bold border mt-10"
            >
              저장하기
            </button>
          </Box>
